module datapath(clk, rst, datapath_signals, immediate_data);
    input clk, rst;
    input[3:0] immediate_data;
    input[13:0] datapath_signals;
    
    wire done, extern, AddSub, G_out, G_en, A_en, R3_out, R2_out, R1_out, R0_out, R3_en, R2_en, R1_en, R0_en;
    assign {done, extern, AddSub, G_out, G_en, A_en, R3_out, R2_out, R1_out, R0_out, R3_en, R2_en, R1_en, R0_en} = datapath_signals;

    wire[3:0] four_bit_bus, R0_Q, R1_Q, R2_Q, R3_Q, ALU_arg1, ALU_result;
    ...
    four_bit_register A(.clk(clk), .rst(rst), .en(A_en), .D(four_bit_bus), .Q(ALU_arg1));
    ALU adder(.A(ALU_arg1), .B(four_bit_bus), .result(ALU_result));

    four_bit_register G(.clk(clk), .rst(rst), .en(G_en), .D(ALU_result), .Q(G_Q));
