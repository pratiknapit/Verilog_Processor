module datapath(
    ...
    genvar i;
    generate
        for (i=0; i < 15; i=i+1) begin : gen_regs
            sixteenbit_reg_enable my_regs(.clk(clk), .reset(reset), .en(r_en_OH[i]), .d_in(bus), .q_out(register_out[i]));
        end
    endgenerate
    
    generate
        for (i=0; i < 15; i=i+1) begin : gen_tri_bufs
            tri_state_buffer tri_bufs(.en(tri_controller_OH[i]), .in(register_out[i]), .out(bus));
        end
    endgenerate

    wire [15:0] extern_data_padded = {8'd0, extern_data};
    sixteenbit_reg_enable external_reg(.clk(clk), .reset(reset), .en(extern_en), .d_in(extern_data_padded), .q_out(bus));
    ...
    DMEM dmem_memory(.clk(clk), .we(DMEM_out), .re(DMEM_in), .address(DMEM_addr), .d_in(bus), .d_out(bus));
